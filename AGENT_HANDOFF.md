...
3. I3/I4 按原定义不再推进（用户 2026-09-26 决定，改入“对话能力”方向，待修订 DEVELOPMENT_PLAN/ADR-017）；
   若重新设计含资料查询，仍需明确资料源形态（仓库内文档 / 独立 store / 外部系统），与 Web 产品线不并行修改同一真源。
...
- **部署形态收敛为两个容器（用户 2026-09-25 已定方向，暂缓实施）**：`postgres` + 一个 `xiaowei`
  ...
  现有分容器编排保留。实施前须先写详细计划并获批；只先覆盖本机完整体验，release 形态另需 GO。
