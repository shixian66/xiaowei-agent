...
- **当前阶段：W1a 详细计划送审。** 计划文档在
  ...
  `rev_0014` 或任何 W1a 源码。
- **W1a 按三个切片交付，各自独立复审与合入**：A 契约与迁移、B Store 与审计原子性、
  C 旧身份迁移。依赖单向 A → B → C；前一个合入后后一个才开工。
- **W1a 计划合入后唯一获准的下一步是按该计划 TDD 实现切片 A。**
...
> **当前唯一在途项**：W0 文档/ADR 真源收口。W0 合入后唯一获准动作是编写
> **W1a 详细计划**并送审；W1a 计划获批前不得写其测试、migration 或源码。
