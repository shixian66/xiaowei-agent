...
| 阶段 | **M0–M4 已验收并合入 `main`；M4 已归档；M5 候选实现与全调用链自审已完成，等待精确 SHA 外部复审和 Compose 运行验收** |
...
| M5 工作分支 | `claude/m5-api-worker-compose`，从 `fa6039bfbce680c0720606b2c34e556ce06c18f4` 创建；API/CLI/Worker/migration/Compose 已实现，全调用链自审与离线验证已完成，尚未复审或验收 |
...
9. **待外部复审与运行验收：M5** API/CLI/Worker/Compose 候选、Task 13 全调用链自审和四条全项目验证已完成；下一步是精确 SHA 外部复审，以及在有 Docker 的环境运行不可跳过的 Compose smoke。未获得这两项证据前不得写成 M5 已完成。
...
- **M5 候选在 `claude/m5-api-worker-compose` 上完成离线收口验证**：`python -m pytest -q` 为 1685 passed / 152 skipped / 5 warnings；`python -m pytest -m security -q` 为 923 passed / 79 skipped / 835 deselected / 5 warnings；`ruff check .` 通过；`mypy src` 通过（102 个源文件）；文档一致性门 5 passed。152 个全量 skip 均属于未提供 PostgreSQL DSN 的 integration，用例不是 Compose 运行证据。
...
- M5 的静态 Compose 契约证明 API、Worker 使用同一应用镜像，API 仅向宿主 loopback 发布，PostgreSQL 不发布宿主端口，应用容器只读、无 `container_name`、host network、privileged 或 Docker socket；这些结论来自文件与契约测试，不是容器运行事实。
...
- M5 当前开发机没有 Docker，未运行 PostgreSQL/Compose integration、migration、health/readiness、API 提交、Worker 消费、重启恢复、双 Worker 并发和 CLI 容器调用；CI、部署、canary 与用户验收也均未执行。
...
- M5 的事务结果未知分类与 fencing/接管由 fake、单元和契约测试承重；当前本机未给出真实 PostgreSQL 连接中断、并发锁竞争和进程 SIGKILL 的运行证据。
