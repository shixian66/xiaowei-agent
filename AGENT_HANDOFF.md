...
## 0. 当前事实：W1a 写内核三个切片已全部合入，W1b 尚未实现

- **当前基线**：`main@f92aa88b9016b52c35ebce4052a8609fd574d8fb`（PR #65 squash 合入）。
...
- **W1b 身份激活与通知尚未实现**：没有 `ActivationRequest`、没有 `ActivationStore`、
  ...
  **该计划本身也还没有获批**；获批前不得写它的测试、migration 或源码。
  ...
  整份 W1b 计划仍待 Claude 按精确 SHA 复审，尚未取得实现开工授权。计划中的 1024 上限只约束
  活跃待办；终态申请的受控 PII 保留/清理是 W5 部署硬门，未完成前不得部署启用。
...
| 项目 | 当前值 |
| --- | --- |
| 项目目录 | 当前开发分支 `claude/w1b-activation-plan`（W1b 详细计划与真源绑定守卫待复审），基线 `main@f92aa88b9016b52c35ebce4052a8609fd574d8fb`。不记录机器专属 worktree 路径——它对下一位实现者没有意义且必然过期 |
| 截止时间 | 2026-09-22（Asia/Shanghai） |
| 阶段 | **Web 产品设计（PR #58）、W0 实施计划（PR #59）与 W0 文档收口（PR #61）只改文档；W1a 详细计划（PR #62）获批后，写内核按切片 A（PR #63）、切片 B（PR #64）、切片 C（PR #65）交付并已全部合入 `main`。下一步是 W1b 身份激活与通知：其详细计划已写出但尚未获批，批准前不得写它的测试、migration 或源码。I3 延期未取消。真实模型/飞书/StarRocks、部署、canary 与用户验收仍未开放。最强证据仍为 `tests`。** |
...
| 下一步 | 唯一获准动作是让 **W1b 身份激活与通知**的详细计划通过复审并取得负责人批准；W1b 计划获批前不得写其测试、migration 或源码。I3 受治理资料查询延期但未取消，恢复时仍需先拍板资料源形态（仓库内文档 / 独立 store / 外部系统）。真实 Gemini/飞书/StarRocks、部署、canary、UAT、RI3 test-env GO 与 E1 仍保持各自独立硬门 |
...
> **当前唯一在途项**：W1b 详细计划与真源绑定守卫。W1a 三个切片已全部合入 `main`。
> W1b 计划获批前不得写其测试、migration 或源码。
