...
- **M2 分支 `claude/m2-contract-kernel` 在提交 `e776c180` 上四条命令全绿**：`python -m pytest -q` 613 passed；`python -m pytest -m security -q` 472 passed / 141 deselected；`ruff check .` 与 `mypy src` 均通过。**数字绑定到这个确切 SHA**——不写成「本分支有 N 个提交、N 条测试」，那种写法每修订一次就失真一次（本条此前记的 11 个提交 / 450 passed 即已过期）。更早的逐 SHA 结果见各提交信息。
