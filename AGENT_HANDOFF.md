...
## 0. 当前事实：W1a 写内核切片 A/B 已合入、切片 C 在途，其后是 W1b

- **当前基线**：`main@564707d13a8a8d148d1a2402786313bd31d561f9`（PR #64 squash 合入）。
...
- **切片 C 合入后唯一获准的下一步是 W1b 身份激活与通知**，并需要先有它自己的详细计划与批准；
  计划获批前不得写它的测试、migration 或源码。
...
| 项目 | 当前值 |
| --- | --- |
| 项目目录 | 当前开发分支 `claude/w1a-slice-c-legacy-identity`（W1a 切片 C 旧身份迁移，待复审），基线 `main@564707d13a8a8d148d1a2402786313bd31d561f9`。不记录机器专属 worktree 路径——它对下一位实现者没有意义且必然过期 |
| 截止时间 | 2026-09-22（Asia/Shanghai） |
| 阶段 | **Web 产品设计（PR #58）、W0 实施计划（PR #59）与 W0 文档收口（PR #61）只改文档；W1a 详细计划（PR #62）获批后，写内核按切片 A（PR #63）、切片 B（PR #64）交付并已合入，切片 C 旧身份迁移在途。下一步是 W1b 身份激活与通知，需先有它自己的详细计划与批准。I3 延期未取消。真实模型/飞书/StarRocks、部署、canary 与用户验收仍未开放。最强证据仍为 `tests`。** |
...
| 下一步 | W1a 切片 C 合入后，唯一获准动作是编写 **W1b 身份激活与通知**的详细计划并送审；W1b 计划获批前不得写其测试、migration 或源码。I3 受治理资料查询延期但未取消，恢复时仍需先拍板资料源形态（仓库内文档 / 独立 store / 外部系统）。真实 Gemini/飞书/StarRocks、部署、canary、UAT、RI3 test-env GO 与 E1 仍保持各自独立硬门 |
...
> **当前唯一在途项**：W1a 切片 C 旧身份迁移。它合入后唯一获准动作是编写
> **W1b 详细计划**并送审；W1b 计划获批前不得写其测试、migration 或源码。
