...
| W2 / W3 / W4a/W4b | W2、W3 V1 已离线收口；W4a/W4b 已离线实现并合入。W3 后续增强仍未实现但不阻塞 W5；W4c、真实调用、部署、R1 均未由此解锁 |
...
- **新功能路线：不做旧能力整体迁移（用户 2026-09-26 已定方向，暂缓实施）**：旧小维实现不搬、不以
  “对齐 51 项”为目标，旧能力清单只作需求备忘；按当前需求在新架构上重新设计。第一优先是
  “数据库只读查询 → 审批规则（ADR-005，导出审批与 E1 分开）→ 结果域（R1）→ 导出与导出审批”，
  其次是 Admin 查看任务/聊天内容并写查看审计。当前 main 以上均未实现：M6b 只覆盖慢查询两个固定
  operation，没有通用查询、结果导出或导出审批。方向、依赖与待拍板问题见
  [新功能路线方向](docs/superpowers/specs/2026-09-26-feature-roadmap-direction.md)；它不是详细计划，
