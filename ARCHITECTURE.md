...
> 当前状态：下方分容器部署形态仍是本节保留的现行架构文本。两容器是负责人已确定但尚未采用的方向；实施前必须修订本节，并决定是否补写或修订 ADR-001。该方向与 RI6 部署均暂缓，分别需要详细计划和负责人 GO；事实与状态见[当前交接](AGENT_HANDOFF.md#current-status)。
