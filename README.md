...
> adapter 已完成离线实现、审查并合入 `main`，真实验证已延期，最强证据仍为 `tests`。M7 只完成
> V0.6 阶段门与产品范围文档修订，源码尚未开始；PR 1–8 均可在文档合入并取得明确口令后离线
> 开发，真实应用、凭据、网络连接、部署与 canary 仍被独立硬门阻塞。项目**尚未连接任何真实
