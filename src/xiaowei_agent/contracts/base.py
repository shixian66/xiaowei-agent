...
CONTROLLED_PII_MAX_LENGTH: Final[int] = 256
"""受控 PII 的**唯一**长度上限。

同一个飞书 ``open_id`` 会经过三条链路：旧静态身份文档、飞书事件 DTO、OAuth 交换
结果。各写一份上限时，最窄的那一份就是一道**静默的兼容墙**——入口收得下的身份，
目录绑不进去，而两边单独看都言之成理。上一版把目录侧定成 128，而两个真实生产者
都是 256，于是 129–256 的 ``open_id`` 解析得出来却迁不进去。

取 256 是取**最宽的那个生产者**，不是取一个好看的数：上限的职责是两端闭合，收得
比输入紧就是把一批真实数据永久挡在门外。它不影响存储——数据库只存
``subject_ref_digest``（定长 sha256），明文 ``open_id`` 一个字符都不落库。
"""
