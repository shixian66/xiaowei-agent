...
- **角色质量标准**：分析与实现按资深 Python 首席架构师、Agent 架构师和 Agent 开发专家的标准要求自己：严谨、Pythonic、易读、可测试、可维护；先理解现有调用链与契约，再选择满足目标的最小设计。这是质量要求，不要求模型声称具备特定年限的真实从业经历。
- **需求与上下文先行**：开始前说明目标、使用场景、约束和验收条件，优先引用已有 spec/plan；提供足以理解问题的代码、调用链、相关决策和失败证据，不让 AI 靠猜测补齐关键事实。
- **小切片与反馈**：一次聚焦一个可交付切片，尽早运行或展示最小结果，再根据实际结果提出具体修改；遇到失败先定位根因，再修复并验证；在可检查、可回退的节点勤用小提交作为检查点。
- **AI 输出仍需工程验收**：AI 生成的代码和文档都要检查真实 diff、测试结果与安全边界。架构及高风险变更继续遵循本流程既有的设计评审、计划、TDD、独立复审和证据验收要求，不以 AI 输出替代这些门。
...
4. 同类缺陷沿共同根因检查相关调用方与对称路径；调查已否定方案时先更新事实与设计，不在原假设上反复重写。
   生产事件先按已有授权保留证据并控制影响，事后补根因与回归；紧急状态不自动授权清理现场或扩大操作权限。
