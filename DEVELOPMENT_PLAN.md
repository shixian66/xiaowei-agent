...
> 状态：路线 V3.1（2026-09-26）。在 V3.0 基础上补充对话能力与两容器 → RI6 后续轨道、负责人本机 Gemini 体验的证据边界及 RI3 PR 3E 延后口径；两容器可由负责人按需提前启动，不依赖 F1。既有 F1–F5 顺序与 M/W/RI 阶段门不变；本次修订不授权源码、真实调用、部署或 E1。
> V3.0 于 2026-09-26 记录负责人确认的新功能路线与对话能力方向；本版只细化后续规划门，不改变此前批准。
> V2.6 于 2026-09-24 按项目负责人明确决定，把已经交付的 W3-lite 固化为 **W3 V1**；原 W3 中尚未
> 实现的职责绑定、可靠/私聊通知与敏感内容查看归入 **W3 后续增强**，延期但未取消，且不作为
> W4a/W4b 的进入条件。以下 V2.5/V2.4 记录为历史决策，不覆盖本次 V3.1 路线修订。
> V2.5 于 2026-09-20 增加 Web 产品线 `W0 → W5` 交付序列，并把当前优先级从 I3 切换为 `W0 → W1a`
> （当时记录为 I3 延期）。批准来源：项目负责人决策记录
> <https://github.com/shixian66/xiaowei-agent/pull/59#issuecomment-5750387268>。
> V2.5 的原始批准当时只授权 W0 文档/ADR 收口，并未授权 W1a 源码、迁移、真实调用、部署或
> 用户验收；W1a–W3 后续均由各自详细计划与开工口令独立授权，不能反向扩大那次批准。
...
> [ADR-017](docs/adr/ADR-017-intelligent-interaction-and-clarification.md) 是智能交互入口的 I1 实施门；I1/I2
> 已离线实现，I3/I4 原定义按 2026-09-26 决定不再推进，改为尚未设计的无编号对话能力方向。
...
**智能交互路线状态**（ADR-017）独立于 RI2/RI3/RI4 的真实接入 GO：I0–I2 保持原有范围，I1/I2
已离线实现。I3（受治理资料查询）与 I4（用户提供日志分析）的旧定义不再推进；由尚未设计、无编号的
“对话能力”方向重新规划。I5 真实模型 Eval/灰度/UAT 不由本路线自动开放，仍需结合未来计划单独定范围与授权。
多轮对话记忆、跨 capability 组合和证据驱动的自主调查均未获批准。该方向不构成 M8 前置，也不改变 RI2
飞书、RI3 Gemini、RI4 StarRocks、H 层生产只读、部署/canary 或 E1 的任何真实调用 GO。

**新业务功能路线（F1–F5）**与 W0–W5、M8/E1 及 RI 部署路线分开排期，详细范围见
[新功能路线方向](docs/superpowers/specs/2026-09-26-feature-roadmap-direction.md)：F1 为单个已授权
StarRocks target 的全库/全表只读查询与锁定结果页；已激活、已认证且能提交只读任务的用户均可查询，不另设逐人许可，
查询本身无需审批；F2 单独审批 Web 详细结果查看；F3 再单独审批 CSV 受控下载，查看与下载互不授权，CSV 保留 1 天；
两类审批权限分别由 Admin 分配给具体用户，默认无人拥有；每类由一名具备相应权限的人批准，申请人仅在本人也获配该权限时可自审，审批有效期各 1 小时。F1 结果 artifact 保留 1 天；F4 是
StarRocks 路线验证后单独接入 TiDB；F5 Admin 历史查看、聊天记录与多轮记忆暂缓。F1 设 `max_rows`、`max_bytes`、
`query_timeout` 并在命中任一上限时明确标记不完整，具体数值进入 F1 详细计划。该方向已确认不等于实现授权；
每项仍须详细计划、exact-SHA 复审与明确开工口令，真实 StarRocks 调用另需现场 GO。
...
| 结果访问与导出审批 | F2 Web 结果查看和 F3 CSV 下载分别走飞书审批；对应审批权限由 Admin 分配给具体用户，默认无人持有；每项由一名获配相应权限的人批准，申请人仅在本人获配相应权限时可自审；审批有效期各 1 小时，两项授权不互相推导。拒绝/过期/冲突与重放契约由各自详细计划及 ADR-005 固化，不授予 E1 | F2 / F3 详细计划 | 对应审批未完成前不开放相应结果查看或 CSV 下载 |
| E1 写审批 | M8 仍须单独确定主体、渠道、有效期、拒绝/过期/冲突语义，并满足本表写操作许可的全部独立门 | M8 | 不开放 E1 |
| 模型供应商 | 核心测试继续使用 fake interpreter；RI3 固定 Google Gemini Developer API `v1beta`、canonical origin `https://generativelanguage.googleapis.com`、`gemini-3-flash-preview` 和官方 `google-genai==2.23.0` 的异步 `models.generate_content`；实现 PR 必须独立审计精确 wheel，任何身份或元数据不匹配都停下复审。Gemini 明文在 RI5 阶段的唯一真源是宿主 Git-ignored 的 `.config/integrations.json`（容器内 `/run/xiaowei-config/integrations.json`）；W4a 起按 ADR-015 修订改为 AI 域 `.config/ai/config.json`，旧单文件只作显式一次性迁移输入。它由 Web 管理面写入、需要凭据的进程只读挂载；不把 key 或宿主路径放进 `.env`。不设本地费用硬封顶，但保留调用次数、输入/输出边界和 usage 观测；实现权不等于真实调用权，现场调用另需 RI3 GO | ADR-015/V7.1 计划已批准离线实现；真实调用现场门未开放 | 无现场 GO 不读取真实 key、不联网 |
...
| I3（旧定义，退役） | Intelligent Interaction | 原“受治理资料查询”不再按原路线推进；归入尚待设计的无编号对话能力方向 |
| I4（旧定义，退役） | Intelligent Interaction | 原“用户提供日志分析”不再按原路线推进；归入尚待设计的无编号对话能力方向 |
| I5 真实模型 Eval / 灰度 / UAT | Intelligent Interaction | 未来方向；须结合对话能力计划重新界定，不能替代 RI3 首次 Gemini GO |
...
新业务路线 F1–F5 是独立的后续功能轨道，不插入既有 M/W/RI 阶段编号，也不自动修改其退出门；它也不属于
W0–W5 发布序列。F1–F3 按依赖先后，F4 待 StarRocks 验证后再单独启动，F5 暂缓。具体范围和逐项开工门见
第 3.1 节及关联路线规格。

| 序 | 范围 | 进入与边界 |
| --- | --- | --- |
| F1 StarRocks 查询与锁定结果页 | 单个已授权 target 使用 Admin 配置的 DBA 级只读凭据访问其可达的全部数据库和表；任何已激活、已认证且能提交只读任务的用户均可查询，不另设逐人许可；飞书只发受保护 Web 链接、不放真实结果行；结果 artifact 保留 1 天。 | 查询无需审批；Web 详细结果需 F2 单独审批，批准后申请人和该次查看审批人可访问；确定性 SQL + AST；`max_rows`、`max_bytes`、`query_timeout` 必须有值，触顶显式标记不完整；真实目标调用单独现场 GO。 |
| F2 Web 详细结果查看审批 | 首期飞书；审批权限由 Admin 分配给具体用户，默认无人持有；获配相应权限的一人即可，申请人仅在本人也获配该权限时可自审，审批有效期 1 小时。 | 仅授权 Web 结果查看，不授权 CSV 下载或 E1；失败、过期、冲突语义须在详细计划与 ADR-005 中固化。 |
| F3 CSV 下载审批与导出 | CSV 唯一格式；下载另走独立飞书审批，审批权限由 Admin 分配给具体用户，默认无人持有；获配相应权限的一人即可，申请人仅在本人也获配该权限时可自审，审批有效期 1 小时；通过后由 Web 受控下载，文件保留 1 天。 | 不因 F2 查看审批自动开放导出，反之亦然；下载授权时限和文件/行数预算在计划中确定；服从 F1 资源上限和完整性标识。 |
| F4 TiDB | StarRocks 路线验证后作为独立增量接入。 | 单独的 capability / 方言校验 / 测试和真实调用许可；不由 StarRocks 支持推导。 |
| F5 Admin 历史查看、聊天记录与多轮记忆 | 暂缓。 | 不新增会话存储、Admin 查看任务/聊天内容或隐式记忆；恢复时另立数据与访问设计。 |

### 两条后续规划轨道（不新增既有阶段编号）

下列方向已确认，但均暂缓实施；不插入或重排 F1–F5、M、W、RI、I 阶段，也不改变任何既有退出门。每条轨道在实施前都必须有自己的详细计划、exact-SHA 独立复审和负责人明确 GO。

| 轨道 | 规划顺序与交付 | 进入与边界 |
| --- | --- | --- |
| 对话能力（无编号，替代 I3/I4 原规划） | 先完成产品范围与架构设计，明确自然沟通、缺槽澄清、任务进度/结果说明，以及通用、知识、运维、诊断请求分流的用户场景和验收/Eval；设计获批后，再另写实现计划并申请开工。 | 不恢复 I3/I4 旧定义，不新增 F 编号，不改变 F1–F5 顺序。首期不含隐式多轮记忆；模型不能自行选择工具或执行。跨 capability 组合与证据驱动的自主调查须另立 ADR 并获批准后才能实现。 |
| 两容器本机形态 → RI6 | 先为本机目标形态 `postgres` + 一个 `xiaowei` 容器编写独立详细计划并完成本机验证；本机验收后，再为 RI6 正式部署、canary 与 UAT 编写单独计划。 | 两容器方向尚未采用。负责人可按需提前启动该轨道，不依赖 F1；提前只调整启动时序，不豁免独立详细计划、本机验收与负责人 GO。开始本机实施前，必须修订 `ARCHITECTURE.md` §11，并决定是否补写或修订 ADR-001。本机详细计划与 RI6 计划分别需要负责人 GO；本机验证不授权正式部署、真实调用、canary、UAT 或 E1。W5-C 已取消，W5 离线资产是否复用由对应详细计划决定。 |

**对话能力与 RI3 的关系（负责人 2026-09-26 决定）**：负责人报告已在本机通过 Web 与飞书群 @小维手动测试真实 Gemini：慢查询诊断的意图识别与模型分析、能力清单问答均正常。该结果仅是负责人本机体验观察，不是 RI3 `test-env verified` 证据；对话能力开工不以正式 RI3 验证为前置。正式 RI3 验证（PR 3E）推迟至验收阶段；届时须先刷新 RI3 计划 Task 5，使 Gemini key 按当前配置契约从 `.config/ai/config.json` 加载，并按 ADR-017 当前入口改写旧 20 轮父链注入测试。现场执行仍须负责人明确 GO。
...
- **W3 V1 用户 / 激活 / 审计 UI**：增加用户、待激活申请与 Admin 审计查询 UI，并把角色、状态和
  激活决定作为受管授权变更通过唯一授权写入口完成；不新增第二条授权写路径。
- **W4a AI / 飞书配置迁移**：迁移到三域配置文件与进程挂载矩阵，新旧并存时 fail-closed 为
  `migration_required`。
- **W4b 数据库 / Prometheus 参数登记**：只保存参数并做本地校验，**网络调用为 0**。
- **W5 产品部署**：release override、可审计开关、边缘限流、配置目录预检与分级运行证据。**W5-C 已于 2026-09-26 取消；W5-A/B 资产保留备用，不是当前部署入口。**

**W3 后续增强**包括 DBA/值班/激活通知人职责绑定、群内明确 @ Admin、私聊/可靠通知、持久化
投递状态与重试/dead-letter，以及私人或敏感任务内容查看和查看审计。这些能力延期但未取消，须各自
重新形成增量计划与授权；它们**不作为 W4a/W4b 或 W5 的进入条件**。W4b 因而只登记资源参数，
不得假设已有 `RoleAndDutyService` 或把职责绑定暗藏进资源文件。

### 独立阻塞门（不在上述必经序列内）

- **W4c 运维目标纳入 Web Admin 测试范围**：必须另修 ADR-007、指定唯一 task-worker 路径并取得
  现场 GO。
- **W0–W5 结果域排除**：本 Web 产品线不创建数据库结果 artifact、预览、导出或结果服务。F1–F3 是独立新功能路线，
  R1 不再作为独立功能门；F1–F3 承接结果查询、访问和导出方向，但不属于 W0–W5，也不代表完整通用 R1 平台获批。
  Web 规格 §11.2 的六项前置条件仍然有效；
  F1 详细计划必须逐项满足并给出验收安排，或说明不满足项的变更及理由。Admin 不因角色自动越权。

**I3/I4 原定义不再推进**，归入尚待设计的无编号对话能力方向；它与新业务功能路线、Web 部署路线分别规划。
...
智能交互路线只改变入口能力；离线实现全绿不能写成真实模型、真实飞书、真实目标、部署、灰度或用户验收。
...
本节的 `api`、`worker`、`postgres` 是 M5 的既有里程碑范围，不表示当前完整 Compose 只有这三个服务，也不因后续两容器方向而追溯改写 M5。两容器是后续独立的本机形态收敛轨道，进入门见第 6 节。
...
**凭证与配置**：RI5 阶段唯一明文来源是宿主 Git-ignored 的 `.config/integrations.json`
（容器内 `/run/xiaowei-config/integrations.json`），由 Web 管理面写入，需要凭据的进程只读挂载，
`api` 不挂载。W4a 起按 ADR-015 修订拆为 `ai` / `feishu` / `resources` 三个固定配置域并按域挂载
（矩阵见 ARCHITECTURE），旧单文件只作显式一次性迁移输入，运行时不双读。Provider 凭据不再走 Docker secret。不提供宿主路径环境变量覆盖，渲染后的 Compose
...
**部署形态顺序**：RI6 的目标形态为先经独立本机计划与验收的 `postgres` + `xiaowei` 两容器形态。两容器本机验证与 RI6 正式部署各有独立详细计划和负责人 GO；本机验收不能替代 RI6 授权。两容器实施前还须按第 6 节修订 `ARCHITECTURE.md` §11，并决定是否补写或修订 ADR-001。W5-C 已取消，保留的 W5 资产不能作为当前部署入口；是否复用须在两容器或 RI6 详细计划中逐项确认。
