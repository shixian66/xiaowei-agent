...
> 状态：Approved V2.1。V2 于 2026-09-01 由项目负责人批准；V2.1 于 2026-09-07 仅为 M7
> 详细计划中标注为「离线实现门」的基础工作增加窄例外，仍不授权直接开始业务实现。每个阶段门
> 仍需详细计划、文档基线与明确开工口令。
...
**M7 窄例外（2026-09-07，经项目负责人明确批准）**：M6b 真实验证延期期间，M7 仅可提前
实施详细计划中标注为「离线实现门」的基础工作——当前 V0.5 对应 PR 1–3，包括渠道契约/ADR、
无执行权的任务读取投影重构，以及 TaskStore/ChannelStore/访问与提交服务。该例外不允许安装飞书
SDK、实现或启动真实渠道进程、注册应用、调用飞书或任何真实运维目标，也不改变 M6b 的证据等级。
详细计划中标注为「真实渠道门」的工作——当前 V0.5 对应 PR 4–8——仍须先满足 M6 真实只读
canary、真实渠道授权和重新核验条件；M7 最终退出标准不降低。PR 编号仅是当前版本映射，阶段门
标签才是本总体计划的规范锚点。
...
**分阶段进入条件**：详细计划中标注为「离线实现门」的基础工作（当前 V0.5 对应 PR 1–3）可按
第 3.1 节已批准的窄例外离线实施，但仍须先把门槛修订作为纯文档基线合入、重新读取最新文档与
Git/前序证据，并取得项目负责人明确的离线开工口令。标注为「真实渠道门」的工作（当前 V0.5
对应 PR 4–8）不适用该例外；其阶段门未全部满足前保持阻塞。
...
为「离线实现门」的基础工作（当前 V0.5 对应 PR 1–3）；它不解锁标注为「真实渠道门」的工作
（当前 V0.5 对应 PR 4–8）、不改变 M6b 状态、不授权真实调用，也不降低 M7 退出标准。M2 及以后
只保留路线级定义，不提前创建超出已批准阶段门的代码或依赖。
