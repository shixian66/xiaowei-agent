...
本计划把 [ARCHITECTURE.md](ARCHITECTURE.md) 的目标架构拆成可独立验收的里程碑。四类文档各自只承担一种责任：
...
本计划经项目负责人批准后，只进入 M0：先检查忽略规则和敏感信息，初始化本地 Git 基线并记录 SHA，再在 `claude/m0-plan-closure` 分支完成文档/ADR 收口。M0 通过后才为 M1 编写详细实施计划；M2 及以后只保留路线级定义，不提前创建代码或依赖。
