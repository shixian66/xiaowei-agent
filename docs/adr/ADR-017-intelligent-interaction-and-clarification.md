...
- I0–I2 保持原范围；I1/I2 的离线实现状态与证据以当前 handoff 为准。
- 原 I3（受治理资料查询）与 I4（用户提供日志分析）不再作为现行待办路线。后续对话能力方向不新增阶段编号，须按当前需求另行设计、形成计划并获批。
- 本修订不授权多轮对话记忆、跨 capability 组合、证据驱动的自主调查、任何新增工具或真实连接。执行仍服从现有确定性路由、StepAdmission、Policy 与工具边界。
- 原决策正文保留为 I1/I2 的设计背景与历史契约；本段不重写其决策条款。
