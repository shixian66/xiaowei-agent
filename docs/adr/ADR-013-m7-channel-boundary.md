...
**W3 V1** 增加用户、待激活申请与 Admin 审计查询 UI，并只经 W1a 的唯一授权写入口执行角色、
状态和激活等受管授权变更，不新增第二条授权写路径。敏感内容查看审计与职责绑定属于 W3 后续增强。

### W3 V1 与职责范围

W3 V1 不交付 DBA、值班、激活通知三类职责绑定。这些能力作为 **W3 后续增强** 延期但未取消，
须另行计划与授权，且**不作为 W4a/W4b 或 W5 的进入条件**。W4b 只登记资源参数，不能把职责绑定
写入资源配置文件。**requester/approver** 名单与数据库结果 ACL 只属于未来 **R1**，不在
W0–W5 内交付。
