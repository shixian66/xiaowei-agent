...
`AGENTS.md` 已把 pytest 和 `security` marker gate 定为基线，但四份文档中同时存在缺少 `python -m` 前缀的调用形式与规范形式，违反单一真源原则；缺少前缀的调用在解释器不确定时会取到错误环境。`README.md` 另有把 Python 版本写成开放区间的表述，隐含承诺了没有任何验证证据的更高版本兼容性。
...
- 四份既有文档中 7 处缺少 `python -m` 前缀的命令需在 M0 同步修正，属本 ADR 的直接后果。
