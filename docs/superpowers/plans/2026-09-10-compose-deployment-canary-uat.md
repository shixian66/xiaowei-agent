# Compose Deployment, Canary, and UAT Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan.

**Goal:** 用 Docker Compose 把已通过前序门的精确制品部署到正式主机，完成回滚演练、小范围 canary 和产品用户验收，并严格分开各级证据。

**Architecture:** 继续一个镜像、多进程 Compose：PostgreSQL、migrate、内部 API、task worker、Web、飞书 listener、channel worker。容器 Web 只提供 HTTP `IP:8080`；项目不部署 TLS/Ingress，OAuth 继续依赖已有 HTTPS SSO 域名。镜像 digest、配置版本和进程 readback 共同证明运行的是哪个版本。
...
**Global Constraints:** 本计划不授权部署。正式主机、变更窗口、canary 用户、验收人和回滚负责人都必须由项目负责人明确。旧小维与 2.0 机器人不得同时运行；不新增 Kubernetes、Ingress、TLS 容器或云部署平台。
...
- 飞书和 StarRocks 至少具有 `test-env verified`；模型若启用也必须有独立 test-env 证据。未验证 provider 必须保持关闭。
- Admin active config、secret references 和目标进程 loaded readback 在测试环境一致。
- 项目负责人指定正式主机、网络边界、Compose project name、数据备份责任人、部署窗口、canary 用户/群、观察窗口、UAT 验收人和旧小维恢复方式。
...
- 不在正式主机 build 源码；
- Web 发布 `8080:8080`，内部 API 默认仍只绑定 loopback 或 Compose network；
...
只覆盖镜像引用、正式端口、资源/重启策略和只读 config/secret mounts；不复制整份 base Compose，不新增代理容器。

**Step 3: 提交 Compose 制品**

```bash
git add docker-compose.yml docker-compose.production.yml tests/contract/test_compose_contract.py tests/contract/test_compose_smoke_script.py tests/contract/test_deployment_artifact_contract.py README.md
...
项目负责人确认精确 SHA、image digest、active config version、secret reference 状态、部署窗口和回滚负责人。没有 GO 不连接正式主机。
