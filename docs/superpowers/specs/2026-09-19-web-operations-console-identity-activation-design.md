...
- 未来 F1–F3 数据库结果链接若成为首次入口，仍复用同一激活服务；在结果域按计划落地前不注册结果路由。

### 2.3 权限和数据库结果

- DBA、值班人员不是第四、第五个全局角色，而是资源范围内的职责绑定。查询提交者、结果查看审批人和 CSV 审批人
  也不是全局角色；F1–F3 按任务/结果记录相应关系，W0–W5 不为此建表或开放管理入口。
- 既有 `ChannelPermission` 三成员闭集保持不变；Admin 管理能力使用独立 `AdminCapability`，不把配置、
  审计等权限硬塞进渠道权限。
- 群成员可查看该群任务的完整**安全证据摘要**和“还有更多证据”提示；数据库真实结果行不进入群卡片。
- 未来数据库结果详情与导出只能按该任务对应的申请人及授权关系访问；F1–F3 必须分别落实 Web 查看和 CSV 下载授权，不能相互推导。
...
查询发起不新增逐人白名单；结果查看与 CSV 导出审批关系必须随 F1–F3 的结果域和 requester/approver ACL 真源一起设计、交付。
...
| `viewer` / `approver` | `USER` | 只迁移产品角色；旧 approver 标签列入迁移报告，F1–F3 详细计划冻结 ACL 真源前不映射为当前授权 |
...
2. **用户与权限**：W3 V1 提供用户、角色、状态、飞书绑定和待激活申请；DBA/值班范围与激活通知人
   属于 W3 后续增强；结果查看/CSV 审批权限须由 F1–F3 详细计划确认真实消费者后再设计管理入口；
...
不在 W4b 预存，随 F1–F3 的结果 ACL 真源一起实现。
...
F1–F3 新功能方向已获确认，但未获源码、真实调用、数据展示或导出授权。以下六项仍是数据库结果访问与导出的强制条件：
F1 详细计划必须逐项说明设计、负责阶段和可验收证据；其中由 F1 实现的结果 artifact 与 ACL 可列为 F1 交付物，
但必须在开放相应 Web 结果访问或 CSV 导出路径前验收通过。任何不能满足或拟变更的条目，都必须写明变更范围、理由与影响并先获批准，
不能因 F1–F3 方向已确认而默认为豁免。
...
适用的阶段验收未通过时：不向用户开放 `/results/{result_ref}`、`ResultAccessService`、结果页或导出接口；不得用空表或 fake 结果页冒充交付。
...
| `RoleAndDutyService`（W3 后续增强） | 未来承载 DBA/值班与激活通知绑定；F1–F3 的 requester/approver ACL 由结果域另行定义 | W3 V1/W4b 提前创建、自定义策略表达式、结果 ACL 猜测 |
...
- W3 后续增强中的 DBA/值班/激活通知范围改变；F1–F3 另记录 requester/approver 结果访问和导出审批事件；
...
8. **W5 产品部署**：release override、listener/channel-worker 可审计开关、边缘限流、配置目录预检、
   明确重启、加载回执、浏览器视觉和分级运行证据。W5 只依赖 W2–W4b，不以 W4c 或 F1–F3 为必经前置。
...
- **R1 未来数据库结果访问**：本规格最初将其列为独立结果功能门；2026-09-26 的 F1–F3 方向取代该独立排期门，
  但不改变其不属于 W0–W5 的边界，也不由本规格审核自动授权。

**路线修订（2026-09-26）**：上述“W0–W5 不创建结果 artifact / 结果服务”边界不变。F1–F3 是 W0–W5 之外的
独立业务路线：F1 产出受保护的最小结果 artifact 与锁定的 Web 结果页；F2 单独审批 Web 详细结果查看；F3 单独审批
CSV 下载。飞书卡片和普通 `RenderPayload` 永不嵌入真实结果行，飞书只投影受保护的 Web 结果链接。已激活、已认证且
具备只读任务提交权限的用户可发起查询，不另设逐人查询名单；查询执行本身无需审批。F2 Web 结果查看审批权限与 F3 CSV 下载审批权限
由 Admin 分配给具体用户，默认无人拥有对应权限；每项由一名获配相应权限的人批准，申请人仅在本人也获配对应权限时可自审，审批有效期各 1 小时；两项授权互不推导。F2 批准后申请人和该次查看审批人可按 §11.1
及 §11.3 查看结果，Admin 不因角色自动越权。所有功能仍须单独写详细计划、评审并取得开工授权；F1 详细计划必须逐项
满足 §11.2 的六项前置条件，或说明不满足项的变更及理由。导出审批不代表 E1 写权限。

每个阶段都从最新 `main` 开独立分支，先写详细计划并审核，再按 TDD 实现。W0 合入前不得启动 W1；
W4c、W5 的真实部署/验证和 F1–F3 各自需要新的明确口令。任何阶段都不得为了 UI 完整度伪造数据库结果页面。
...
- `DEVELOPMENT_PLAN.md`：当时把 §17.1 的 W0–W5 建成可独立验收的交付序列，并把 W4c/R1 单列为阻塞门；
  其中 R1 独立结果功能门已由 2026-09-26 的 F1–F3 路线取代，W0–W5 结果域排除继续有效；
- `README.md`：更新登录、初始化、Admin 配置和产品部署路径；
- `AGENT_HANDOFF.md`：修正 I2 已合入、RI5 配置真源等漂移，只记录实际合入、测试、部署与未覆盖证据。

ADR-005 不在 W0 被臆造完成；其审批主体、有效期、拒绝/过期/冲突和审计语义须由 F2/F3 详细计划冻结。上述文档修订必须先于
...
- 截至 2026-09-20，接受数据库结果 ACL 规则保留、R1 延后，且当时不实现 `/results`、预览、导出或结果服务；
  2026-09-26 的 F1–F3 路线取代 R1 独立结果功能门，但仍不代表实现已获授权；
- 截至 2026-09-20，接受 §17.1 的 W0–W5 为交付序列、§17.2 的 W4c/R1 为独立阻塞门，不把后者排成产品部署必经步骤，
  也不把设计稿记为已实现；2026-09-26 后 R1 不再作为独立功能门，W4c 门保持不变；
