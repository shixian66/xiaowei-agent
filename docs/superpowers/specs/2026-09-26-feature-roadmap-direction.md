# 小维新功能与平台方向（Direction V0.2，已定方向、暂缓实施）
...
5. **部署形态方向**：收敛为两个容器（`postgres` + 一个 `xiaowei`），完整口径与已接受的权衡以
   [当前状态](../../../AGENT_HANDOFF.md#current-status) 第 6 节“部署形态收敛为两个容器”一条为准，
   本文不重复展开。`ARCHITECTURE.md` §11 与 `DEVELOPMENT_PLAN.md` M5/RI6 仍按分容器描述，是现行文本；
   开工前须同步修订它们，并明确是否需要补写或修订 ADR-001（模块化单体与 Compose 部署边界，当前尚未
   成文）。要点：先只覆盖本机完整体验，release 形态另需 GO；公司 SSO 免二次登录另立项；实施前须先写
   详细计划并获批。W5-C（provider-off 产品壳部署）已于 2026-09-26 取消，部署路线改为“两容器 → RI6”，
   原 W5-C 须证明的目标环境事项转入该正式部署。
6. **对话能力横向方向（替代原 I3/I4 规划）**：提升小维与用户的自然沟通、缺槽澄清、进度/结果说明，
   以及通用、知识、运维和诊断请求的分流能力。它不新增 F 编号，不改变 F1–F5 的依赖顺序。
   - **用户 2026-09-26 决定**：`DEVELOPMENT_PLAN.md` / ADR-017 中原定的 I3（受治理资料查询）与
     I4（用户提供日志分析）不再按原定义推进，改由本方向按当前需求重新设计；I0–I2 已完成部分不受影响。
     `DEVELOPMENT_PLAN.md` §3.1 的 I0–I5 路线与 ADR-017 **尚未修订**，修订前二者仍是现行文本；
     本方向开工前须先修订它们并获批，不能以本文替代。
   - **多轮对话上下文未获批**：按 `ARCHITECTURE.md`，TaskStore 不保存普通对话，澄清父链不能扩展为
     对话记忆。是否记住多轮对话、存在哪、保留多久、谁能查看，须另立存储设计与 ADR 并获批；本文不授权。
     它与 F5“聊天记录”真源应一并拍板，但**不默认共用同一存储或权限模型**：对话记忆服务于小维理解上下文，
     F5 服务于 Admin 审查并须写查看审计，二者的保留期、可见范围与访问审计须分别论证。
   - 涉及真实系统的请求仍必须回到现有确定性安全链，不授权模型直接选择工具、目标或执行；正式实施前需
     单独设计、评测和验收。
...
6. F5 的“聊天记录”指什么：Web/飞书提交给小维的任务原文，还是飞书私聊全部消息？保留多久？与第 1 节第 6 条多轮
   对话上下文一并决定，但不默认共用存储或权限。

## 5. 其余候选（未排序，待用户按需求挑选）

StarRocks 其他运维查询（当前查询、profile、节点、导入、物化视图等）、巡检与报告、Dinky/Flink 只读诊断、
Jenkins 只读状态、告警主动诊断、任务取消/重跑、DBA/值班职责与可靠通知。
每项入选时单独立项，不从本文外推授权。

## 6. 与既有文档的关系

- 除第 1 节第 6 条记录的 I3/I4 方向变更（待正式修订 `DEVELOPMENT_PLAN.md` 与 ADR-017）外，不修改
  `DEVELOPMENT_PLAN.md` 的里程碑与退出门；R1、M8、W4c、RI4 仍是独立阻塞门，本文只给出建议顺序。
