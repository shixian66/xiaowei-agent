...
- 状态：Accepted V2（2026-09-10 项目负责人批准；RI1 仅获默认关闭、无真实调用的离线实现许可）
- 规划基线：`630c859e6985fe048011057989d5ea4d5ebfc367`；RI1 开工基线由 `AGENT_HANDOFF.md` 记录
...
这些未决项不阻止已批准总体设计和 RI1 离线实现，但会阻止对应真实调用或部署。ADR-007 H 层的
生产只读授权仍未签认；RI1 不读取真实 secret、不发起真实网络调用。
