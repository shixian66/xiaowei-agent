...
这是对上一版 M7c 设计的修订，取代其实现口径。本设计已于 2026-09-14 由项目负责人接受，
与 ADR-007/014/015 及总体 spec 的 RI5 修订成套；**接受设计不等于批准实现**，仍须先通过
单独的实现计划复审才能写源码。
...
这不是对现有 ADR 的静默例外。下列**四份**文档已按本设计修订；
...
必须先完成（1–3 已于 2026-09-14 完成）：

1. ~~本设计复审通过~~ —— 已完成；
2. ~~ADR-007、ADR-014、ADR-015 与总体 spec 的 RI5 修订成套重新接受~~ —— 已完成：ADR-007、ADR-014、ADR-015 与总体 spec 的 RI5 修订已由项目负责人于 2026-09-14 成套接受；该接受不授予 RI3 PR 3E 与 RI2 的真实调用 GO。
3. ~~明确 RI3 PR 3E 与 RI2 真实连接 GO 仍是独立现场门~~ —— 已确认，接受 RI5 修订时由项目
   负责人再次明示不授予这两项 GO；
4. 从最新 `main` 创建实现分支 `claude/ri5-local-web-admin`（**待本轮文档合入 `main` 后执行**）；
5. 再单独编写实现计划，并由复审通过后才开始 TDD 写源码。
